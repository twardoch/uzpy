...
def test_cli_verbose_output(sample_project, capsys):
    ...
    assert any(keyword in captured.out for keyword in [
        "Configuration", "Discovery", "Parsing", "Analysis"
    ])
...
def test_cli_statistics_reporting(sample_project, capsys):
    ...
    assert any(keyword in captured.out for keyword in [
        "Summary", "Total", "Found", "Files", "Constructs"
    ])

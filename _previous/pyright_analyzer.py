...
class PyrightAnalyzer:
    ...
    def __init__(self, project_root: Path, exclude_patterns: list[str] | None = None):
        ...
        try:
            result = subprocess.run(["pyright", "--version"], capture_output=True, text=True, check=False)
    ...
    def find_usages(self, construct: Construct, reference_files: list[Path]) -> list[Reference]:
        ...
        try:
            ...
            cmd = [*self.pyright_cmd, "--outputjson", "--pythonpath", str(self.project_root), str(script_path)]
    ...
    def _parse_pyright_output(self, output: dict, construct: Construct, reference_files: list[Path]) -> list[Reference]:
        """

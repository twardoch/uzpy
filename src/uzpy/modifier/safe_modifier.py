...
class SafeDocstringModifier(cst.CSTTransformer):
    ...
    def _safe_create_docstring(self, content: str, original_quotes: str | None = None) -> str:
        """
        ...
        if original_quotes:
            ...
            if original_quotes == "'''" and "'''" not in content:
                return f"'''{content}'''"
            if original_quotes == '"' and '"' not in content and "\n" not in content:
                return f'"{content}"'
            if original_quotes == "'" and "'" not in content and "\n" not in content:
                return f"'{content}'"
    ...
    def _update_docstring_content(self, current_docstring: str, references: list[Reference]) -> str:
        ...
        content, quote_style = self._extract_docstring_info(cst.SimpleString(current_docstring))
        ...
        if cleaned_content.strip():
            updated_content = f"{cleaned_content.rstrip()}\n\n{usage_section}" if usage_section else cleaned_content
        else:
...
class SafeLibCSTModifier:
    ...
    def modify_file(
        self, file_path: Path, usage_map: dict[Construct, list[Reference]], dry_run: bool = False, backup: bool = True
    ) -> bool:
        """

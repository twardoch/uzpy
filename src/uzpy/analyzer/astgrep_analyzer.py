...
try:
    ...
    try:
        ...
        TreeSitterLang = None
        logger.warning(
            "Failed to import SgRoot from ast_grep_py. AstGrepAnalyzer may not function."
        )
...
class AstGrepAnalyzer:
    ...
    def __init__(self, project_root: Path):
        ...
        if SgRoot is None:
            logger.error(
                "ast-grep Python bindings (SgRoot) not available. AstGrepAnalyzer will be non-functional."
            )
        logger.info(
            f"AstGrepAnalyzer initialized for project root: {self.project_root}"
        )
        ...
        patterns.append(
            {"rule": {"pattern": f"{name}($$$)"}, "description": "Direct call"}
        )
        ...
        patterns.append(
            {
                "rule": {"pattern": f"from $_ import {name}"},
                "description": "Direct import",
            }
        )
        patterns.append(
            {
                "rule": {"pattern": f"from $_ import $MODULE as {name}"},
                "description": "Direct import with alias",
            }
        )
        ...
        patterns.append(
            {
                "rule": {"pattern": f"from $_.$_ import {name}"},
                "description": "Direct import from submodule",
            }
        )
        ...
        if construct.type == ConstructType.MODULE:
            ...
            patterns.append(
                {"rule": {"pattern": f"import {name}"}, "description": "Module import"}
            )
            patterns.append(
                {
                    "rule": {"pattern": f"import {name} as $_"},
                    "description": "Module import with alias",
                }
            )
    ...
    def find_usages(
        self, construct: Construct, search_paths: list[Path]
    ) -> list[Reference]:
        """
        ...
        if SgRoot is None or TreeSitterLang is None:
            logger.error(
                "AstGrepAnalyzer cannot function because SgRoot or TreeSitterLang is not imported."
            )
            return []

        logger.debug(
            f"AstGrepAnalyzer looking for usages of {construct.full_name} in {len(search_paths)} paths."
        )
        ...
        try:
            ...
            logger.error(
                f"Error processing file {file_path} with ast-grep for construct {construct.name}: {e}"
            )

        logger.debug(
            f"AstGrepAnalyzer found {len(references)} references for {construct.full_name}."
        )
        return references
